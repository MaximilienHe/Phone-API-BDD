  ...
  for (let i = 0; i < brands.length; i++) {
    ...
    if (
      ...
      if (apiDevicesCount === bddDevicesCount) {
        ...
        console.log(
          `New devices found for brand ${brand.name}. Nb devices API : ${apiDevicesCount} vs Nb devices BDD : ${bddDevicesCount}\n`
        );
        let cptDevice = 0;
        ...
        while (cptDevice < newDevicesCount) {
          ...
          for (
            let j = 0;
            j < devices.data.length && cptDevice < newDevicesCount;
            j++
          ) {
            ...
            if (!existingDevice) {
              ...
              console.log(
                "Adding device details for device " +
                  device.name +
                  " for brand " +
                  brand.name +
                  " ... \n"
      ...
      new Promise((resolve, reject) => {
        connection.query(sql, [deviceName], (error, results) => {
          if (error) {
            ...
            console.log(
              "Success Calling Stored Procedure for Device ID: " + deviceName
            );
            resolve();
          }
        });
    ...
    new Promise((resolve, reject) => {
      connection.query(sql2, (error, results) => {
        if (error) {
          ...
          console.log(
            "Success Calling Stored Procedure delete_useless_devices"
          );
          resolve();
        }
      });
